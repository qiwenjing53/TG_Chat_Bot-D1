/**
 * Telegram Bot Worker v3.34 (ReCAPTCHA V2 Edition)
 ...
 * | TURNSTILE_SITE_KEY    | (删除)                            |
 * | TURNSTILE_SECRET_KEY  | (删除)                            |
 */
...
export default {
    async fetch(req, env, ctx) {
        ...
        if (req.method === "GET") {
            ...
            if (url.pathname === "/") return new Response("Bot Active (ReCAPTCHA)", { status: 200 });
        }
        if (req.method === "POST") {
...
async function sendStart(id, msg, env) {
    ...
    // 检查 RECAPTCHA_SITE_KEY 是否存在
    if (url && env.RECAPTCHA_SITE_KEY) {
...
// --- 7. 验证逻辑 (已替换为 Google reCAPTCHA V2) ---
async function handleVerifyPage(url, env) {
    const uid = url.searchParams.get('user_id');
    if (!uid || !env.RECAPTCHA_SITE_KEY) return new Response("Miss Config", { status: 400 });
    // 替换为 Google reCAPTCHA 的前端代码
    return new Response(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><script src="https://telegram.org/js/telegram-web-app.js"></script><script src="https://www.google.com/recaptcha/api.js" async defer></script><style>body{display:flex;justify-content:center;align-items:center;height:100vh;background:#fff;font-family:sans-serif}#c{text-align:center;padding:20px;background:#f0f0f0;border-radius:10px}</style></head><body><div id="c"><h3>🛡️ 安全验证</h3><div class="g-recaptcha" data-sitekey="${env.RECAPTCHA_SITE_KEY}" data-callback="S"></div><div id="m"></div></div><script>const tg=window.Telegram.WebApp;tg.ready();function S(t){document.getElementById('m').innerText='验证中...';fetch('/submit_token',{method:'POST',body:JSON.stringify({token:t,userId:'${uid}'})}).then(r=>r.json()).then(d=>{if(d.success){document.getElementById('m').innerText='✅';setTimeout(()=>tg.close(),1000)}else{document.getElementById('m').innerText='❌'}})}</script></body></html>`, { headers: { "Content-Type": "text/html" } });
}

async function handleTokenSubmit(req, env) {
    try {
        const { token, userId } = await req.json();
        // 替换为 Google reCAPTCHA 验证接口
